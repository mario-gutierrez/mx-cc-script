...
| Capability | HID++/transport detail | Client surface |
| --- | --- | --- |
| Friendly device name | HID++ `0x0007` | Device entry `friendlyName` and `nameSource` |
| Special-key reports | HID++ `0x1b04` | Keypad/dialpad key events; reporting is restored on disconnect |
| Dialpad rollers | HID++ `0x4610` | `rollerCapabilities`, `setRollerDiverted()`, `rollerEvent` |
| Keypad brightness | HID++ `0x8040` | `brightnessInfo`, `refreshBrightness()`, `setBrightnessPercent()` |
| Keypad contextual display | MX-specific `0x19a1`, output report `0x14` | `contextualDisplayCaps`, `contextualDisplayInfo`, `uploadContextualDisplayImage()` |
| Normal key input | Raw VLP input report `0x13` | Unified keypad/dialpad key events |

The package intentionally exports only the high-level client, key tables, and roller enum. The feature classes and HID++ transport helpers are internal implementation modules and are not part of the package's public distribution contract.